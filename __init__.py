...
fragment_shader = '''
    ...
    void main() {
        ...
        fragColor = vec4(color.rgb * 1.004, color.a * opacity);
    }
'''
...
def render_snapshot(filepath, time_limit):
    ...
    if temp_camera:
        # 记录临时摄像机的视野框架
        frame = temp_camera.data.view_frame(scene=bpy.context.scene)
        frame_width = abs(frame[0][0] - frame[1][0])
        frame_height = abs(frame[0][1] - frame[2][1])
    else:
        # 记录已有摄像机的视野框架
        frame = original_camera.data.view_frame(scene=bpy.context.scene)
        frame_width = abs(frame[0][0] - frame[1][0])
        frame_height = abs(frame[0][1] - frame[2][1])
    ...
    return frame_width, frame_height, region.width, region.height
...
class OBJECT_OT_TakeSnapshot(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if context.scene.use_full_render and context.space_data.shading.type == 'RENDERED':
            ...
            frame_width, frame_height, region_width, region_height = render_snapshot(filepath, time_limit)
        else:
            bpy.ops.screen.screenshot_area(filepath=filepath)
            frame_width, frame_height, region_width, region_height = None, None, region.width, region.height
        ...
        if not draw_handler.get(area_id):
            draw_handler[area_id] = bpy.types.SpaceView3D.draw_handler_add(draw_snapshot, (area_id, frame_width, frame_height, region_width, region_height), 'WINDOW', 'POST_PIXEL')
...
def draw_snapshot(area_id, frame_width, frame_height, region_width, region_height):
    ...
    if current_area and str(hash(current_area.as_pointer()) % 10000).zfill(4) == area_id:
        if visibility_state.get(area_id) and snapshot_texture.get(area_id):
            ...
            img_width, img_height = snapshot_image[area_id].size
            img_aspect, region_aspect = img_width / img_height, width / height
            if frame_width and frame_height:
                # 计算缩放比例，使渲染结果匹配摄像机的边界
                if region_aspect > frame_width / frame_height:
                    draw_width = width * (frame_width / region_aspect)
                    draw_height = height
                    draw_x = (width - draw_width) / 2
                    draw_y = 0
                else:
                    draw_width = width
                    draw_height = height * (frame_height * region_aspect / frame_width)
                    draw_x = 0
